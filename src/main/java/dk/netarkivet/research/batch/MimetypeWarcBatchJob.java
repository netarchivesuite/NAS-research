 ...
 * 
 */
public class MimetypeWarcBatchJob extends WARCBatchJob {
	/** The class log. */
    private static final Logger log = LoggerFactory.getLogger(MimetypeWarcBatchJob.class);

	/**
	 * Serial version uid.
	 */
	private static final long serialVersionUID = -5915318663120211792L;
	...
	public void processRecord(org.archive.io.warc.WARCRecord record, OutputStream os) {
		try {
			...
			if(mimetype.equalsIgnoreCase("application/http; msgtype=response")) {
				Payload payload = Payload.processPayload(record, record.getHeader().getLength(), 16384, null);
